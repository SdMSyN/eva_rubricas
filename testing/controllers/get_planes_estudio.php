<?php
    ...
    $ban = false;
		...
		$idEsc = $_POST['idEsc'];
    $sqlGetPlanEst = "SELECT $tPlanEst.id as idPlanEst, $tPlanEst.nombre as namePlanEst, "
			." $tPlanEst.year as yearPlanEst, $tPlanEst.estado_id "
			." FROM $tPlanEst "
			." WHERE $tPlanEst.escuela_id = '$idEsc' ";
    ...
    if($resGetProf->num_rows > 0){
        while($rowGetProf = $resGetProf->fetch_assoc()){
            ...
            $planes[] = array('id'=>$id, 'nombre'=>$name, 'year'=>$year, 'edoId'=>$edoId);
            $ban = true;
        }
    }else{
        $ban = false;
